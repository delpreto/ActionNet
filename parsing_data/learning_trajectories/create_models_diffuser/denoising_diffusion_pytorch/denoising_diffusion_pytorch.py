...
class Trainer(object):
    ...
    def train(self):
        ...
        start_training_time_s = time.time()
        while self.step < self.train_num_steps:
            ...
            if self.step % 50 == 0 or self.step == self.train_num_steps-1:
            ...
            if (self.save_every is not None and (self.step+1) % self.save_every == 0) or (self.save_last and (self.step+1) == self.train_num_steps):
                self.save(self.step)
            
            if self.sample_every is not None and ((self.step+1) % self.sample_every == 0 or (self.step+1) == self.train_num_steps):
                ...
                for i, conditions in enumerate(conditions_l):
                    ...
                    if self.renderer is not None:
                        unnormed = self.ds.unnormalize(all_images)
                        savepath = str(self.results_folder / f'sample-{(self.step)}-{i}.png')
                        plot_samples(savepath, unnormed, self.renderer)
