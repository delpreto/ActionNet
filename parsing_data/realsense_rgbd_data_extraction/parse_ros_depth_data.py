...
try:
    ...
    print("Please run this script with the absolute path to a directory with .bag files ending with a backslash (/)")
    sys.exit()
...
try: 
    ...
    print("Please make sure you are in a directory with the 'available_ros_files.txt' configuration file")
    sys.exit()
    
for filename in files:
    if filename[-4:] == '.bag':
        if 'raw' in filename:
            frames = data_generation(data_dir+filename, False, None)
            if frames is not None:
                save_to_hdf5(data_dir+f'{filename[-23:-4]}_cameras.hdf5', ['depth-raw/rgb', 'depth-raw/time_s', 'depth-raw/time_str'], frames)
        else:
            pass
            parsing_chunks, chunk_time, longest_frame = make_parsing_chunks(data_dir+filename, True)
